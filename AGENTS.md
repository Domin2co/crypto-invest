...
불필요한 대규모 리팩터링은 피한다.
- 최상위 디렉터리, backend/frontend/e2e 구조, 모듈, migration, 운영 문서의 추가·이동·삭제가
  발생하면 같은 변경에서 `README.md`의 프로젝트 구조와 관련 설명을 즉시 갱신한다. 변경 뒤에는
  실제 파일 목록과 README를 대조하고 `git diff`로 확인한다.
...
- 모의거래는 전용 API와 가상 지갑으로 제공한다. 전역 거래 모드 설정은 두지 않으며, 실거래는 기본 잠금 상태다.
...
- 테스트·검증에 필요한 로컬 Backend/Frontend와 프로젝트 Docker Compose 서비스의 기동·중지는 에이전트가 별도 확인 없이 판단해 수행한다.
- 기동·중지한 대상, 최종 실행 상태, 실제 검증 결과와 보완점은 작업 완료 보고에 기록한다.

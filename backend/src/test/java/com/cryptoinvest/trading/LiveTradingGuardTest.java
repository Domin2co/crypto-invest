...
class LiveTradingGuardTest {
    ...
    @Test void rejectsUnlessEveryExplicitLiveSwitchAndLimitIsSet() {
        assertThatThrownBy(() -> new LiveTradingGuard(true, true, new BigDecimal("20000")).requireAllowed(plan, policy, BigDecimal.ZERO))
                .hasMessageContaining("LIVE_KILL_SWITCH");
        assertThatThrownBy(() -> new LiveTradingGuard(false, false, new BigDecimal("20000")).requireAllowed(plan, policy, BigDecimal.ZERO))
                .hasMessageContaining("LIVE_TRADING_DISABLED");
        assertThatThrownBy(() -> new LiveTradingGuard(true, false, BigDecimal.ZERO).requireAllowed(plan, policy, BigDecimal.ZERO))
                .hasMessageContaining("LIVE_DAILY_LIMIT_NOT_CONFIGURED");
        assertThatThrownBy(() -> new LiveTradingGuard(true, false, new BigDecimal("15000")).requireAllowed(plan, policy, new BigDecimal("6000")))
                .hasMessageContaining("LIVE_DAILY_LIMIT");
    }

    @Test void globalKillSwitchBlocksEvenWhenTheRiskPolicyAllowsTrading() {
        assertThatThrownBy(() -> new LiveTradingGuard(true, true, new BigDecimal("20000"))
                .requireAllowed(plan, policy, BigDecimal.ZERO))
                .hasMessageContaining("LIVE_KILL_SWITCH");
    }

    @Test void permitsOnlyRiskApprovedPlanBelowDailyLimit() {
        assertThatCode(() -> new LiveTradingGuard(true, false, new BigDecimal("20000")).requireAllowed(plan, policy, BigDecimal.ZERO))

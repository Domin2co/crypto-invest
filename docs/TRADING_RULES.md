...
추천 엔진은 시장 레짐, 자산 점수(-100~100), 신뢰도(0~100), 점수 구간, 데이터 상태와 설명 요인을 분리해 계산한다. BUY/HOLD/REDUCE 평가는 주문 명령이 아니며, 사용자 보유·현금·목표·최대 비중을 적용한 뒤에도 별도의 사용자 승인/자동투자 정책과 RiskEngine을 통과해야 한다. 매도 축소 사유는 익절, 위험감소, 리밸런싱, 추세 이탈, 펀더멘털 악화를 구분한다.

Unavailable·stale·invalid·outlier 지표는 결측 상태와 출처 시각을 기록한다. 값이 없을 때 중립값을 끼워 넣거나 추정하지 않으며, 결측/신선도는 신뢰도를 낮춘다.

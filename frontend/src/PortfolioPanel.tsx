...
export default function PortfolioPanel({ token }: Props) {
  ...
  return <section className="mt-8" aria-labelledby="portfolio-heading">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold text-slate-950" id="portfolio-heading">연동 포트폴리오</h2><p className="mt-1 text-sm text-slate-600">본인 계정의 읽기 전용 잔고와 공개 시세로 계산합니다.</p></div>
      {token && <button className="rounded-lg border border-blue-600 px-4 py-2 text-sm font-bold text-blue-700 disabled:opacity-50" disabled={query.isFetching} onClick={() => query.refetch()} type="button">포트폴리오 새로고침</button>}</div>
...
function Summary({ title, value }: { title: string; value: string }) {
  return <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-sm text-slate-600">{title}</p><strong className="mt-3 block text-2xl text-slate-950">{value}</strong></article>
}
